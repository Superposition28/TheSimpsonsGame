...
def registerNoesisTypes():
    ...
    noesis.registerDebuggerDataHandler("Cybermorph Image", lambda readAddr, resv: debuggerCheckAsset(readAddr, grobjCheckType, ".cmjag_grobj"))
    noesis.registerDebuggerDataHandler("Cybermorph Model", lambda readAddr, resv: debuggerCheckAsset(readAddr, mdlCheckType, ".cmjag_3dobj"))
...
def debuggerCheckAsset(readAddr, checkProc, writeExt):
    retVal = -1
    ...
    try:
        #trick of the debugger implementation - atomic reads will be endian-swapped to the host
        dataSize = noeUnpack("i", noesis.debuggerReadData(readAddr + 2, 4))[0]
        if dataSize > 0 and dataSize < 0x400000:
            data = noesis.debuggerReadData(readAddr, 6 + dataSize)
            _srcSize, decompData = cmGetFileData(0, NoeBitStream(data, NOE_BIGENDIAN))
            if checkProc(decompData):
                retVal = 0
                dstName = noesis.getScenesPath() + "_cybermorph_interpreted" + writeExt
                with open(dstName, "wb") as f:
                    f.write(decompData)
                noesis.openAndRemoveTempFile(dstName)
    except Exception as e:
        print("Exception while interpreting asset:", e)
