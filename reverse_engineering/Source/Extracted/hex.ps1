$basePath = "A:\Dev\Games\TheSimpsonsGame\PAL\reverse_engineering\Source\Extracted"  # Replace with your root directory path
...
foreach ($formatFolder in $formatFolders) {
    ...
    foreach ($file in $files) {
        ...
        # Run Format-Hex on the file (limit to 4096 bytes)
        $hexOutput = Format-Hex -Path $file.FullName -Count 4096 | Out-String
